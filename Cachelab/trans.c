...
void transother(int M, int N, int A[N][M], int B[M][N]) {
	...
	//move through the array by increments of bxb cache size
	for (i = 0; i < N; i += b) {
		for (j = 0; j < M; j += b) {
...
void trans64(int M, int N, int A[N][M], int B[M][N]) {
	...
	// Access each row and column by performing two loops
	for (colRun = 0; colRun < 64; colRun += 8) {
		for (rowRun = 0; rowRun < 64; rowRun += 8) {

			/*
			 The first loop is for a 4 column 8 row of A. In this loop we  use the supporting variables to store all elements of a row. Then we try to transpose with the right positions in B. However, we are doing on a 4row x 8 collumn B to avoid cache miss so not all elements will be transposed correctly. For example A[3][5] cannot be transposed to B[5][3]. Thus, elements which can't be transposed correctly will be stored in another location for later use.
			 */
			for (k = 0; k < 4; k++) {
			...
			// This loop takes care of the remaining elements.
			for (k = 0; k < 3; k++) {
